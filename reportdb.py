...
def create_views():
  ...
  if check_view_exists("v_author_article") == True:
  ...
  if check_view_exists("v_most_viewed_article") == True:
  ...
  if check_view_exists("v_report_errors") == True:
...
def report_http_errors():
  ...
  db = psycopg2.connect("dbname={}".format(DBNAME))
  c = db.cursor()
  c.execute("select friendly_date, cast(round(percent,2) as text)||'%' as error from v_report_errors;")
  rows = c.fetchall()
  db.close()
  return(rows)

def print_report_summary(rows_to_print):
    """print first rows_to_print rows from the table"""
